...
Weryfikacja: `bun test` (testy jednostkowe logiki gry i katalogu potworków), `bun run typecheck`, `bun run check`, panel debug (`?debug` w URL → link „debug" na ekranie głównym: scenariusze-skoki do etapów gry, nawigacja do każdego ekranu, symulacja rund, siatka mastery, ekonomia, wioska, wyprawa, osiągnięcia, kolekcja, import/eksport zapisu JSON) oraz ręczny click-through.

Deploy: push do `main` → GitHub Actions uruchamia `bun test` i `biome ci` (blokują deploy przy błędzie), buduje i publikuje na GitHub Pages (`https://zorzysty.github.io/potworki/`). Nazwa repo jest zaszyta w `base` w `vite.config.ts`. Build emituje service worker PWA (`vite-plugin-pwa` w `vite.config.ts`, `registerType: "autoUpdate"`; rejestracja jawnie w `src/main.tsx` przez `virtual:pwa-register`, bo wstrzyknięty skrypt nie robi reloadu ani nie sprawdza aktualizacji przy powrocie do karty — bez tego dzieci tygodniami grały na starym buildzie; reload po nowym SW jest odkładany, aż `round` i `lastHatch` wrócą do null, bo runda i ekran wyklucia nie są persystowane; offline działa z precache całego builda) — appka jest instalowalna z ekranu głównego (manifest `scope`/`start_url` MUSZĄ siedzieć pod `/potworki/`).
