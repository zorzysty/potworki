...
function litCrystals(progress: number): number {
...
function gateFactor(stage: number): number | undefined {
...
function Crystal({ lit, index }: { lit: boolean; index: number }) {
	const uid = useId()
...
export function MapScreen() {
	...
	useEffect(() => {
		if (!reveal) return
		useGame.getState().markGatesCelebrated() // od razu — animacja nie powtórzy się po wyjściu
					...
					{revealed && (
						<>
							<div className="anim-pop rounded-3xl bg-gradient-to-r from-amber-300 to-orange-400 px-6 py-3 text-2xl font-extrabold text-white shadow-lg">
								Nowa tabliczka: ×{gateFactor(reveal.stage)}! 🎊
							</div>
