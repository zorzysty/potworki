...
describe("MONSTER_COUNT and MONSTERS", () => {
	test("MONSTER_COUNT === 92", () => {
		expect(MONSTER_COUNT).toBe(92)
	})

	test("MONSTERS.length === 92", () => {
		expect(MONSTERS.length).toBe(92)
	})
...
describe("rarity distribution", () => {
	test("common: 36, rare: 21, epic: 11, legendary: 24", () => {
		...
		// 4 oryginalne (45,46,47,71) + 4 tylko-dzielenie (72–75) + 4 tylko-luka
		// (76–79) + 4 tylko-Dzielniki (80–83) + 4 tylko-karmienie (84–87) +
		// 4 tylko-memory (88–91)
		expect(IDS_BY_RARITY.legendary.length).toBe(24)
	})
})

describe("DNA uniqueness", () => {
	test("all 92 DNA signatures are unique", () => {
		const sigs = MONSTERS.map(
			(m) =>
				`${m.dna.body}-${m.dna.palette}-${m.dna.eyes}-${m.dna.mouth}-${m.dna.topper}-${m.dna.pattern}-${m.dna.accessory}`,
		)
		const unique = new Set(sigs)
		expect(unique.size).toBe(92)
	})

	test("all 92 names are unique", () => {
...
describe("division-only legendaries", () => {
	...
	test("macierz pul per tryb: każdy blok ekskluzywny widoczny tylko dla swojego trybu", () => {
		...
		const pairs = idsByRarityForMode("pairs")
		const feed = idsByRarityForMode("feed")
		const memory = idsByRarityForMode("memory")
		expect(feed.legendary.length).toBe(8)
		expect(memory.legendary.length).toBe(8)
		for (const id of MEMORY_ONLY_IDS) {
			expect(rarityOf(id)).toBe("legendary")
			expect(isMemoryOnly(id)).toBe(true)
			expect(mult.legendary).not.toContain(id)
			expect(feed.legendary).not.toContain(id)
			expect(memory.legendary).toContain(id)
		}
		expect([...MEMORY_ONLY_IDS].sort((a, b) => a - b)).toEqual([88, 89, 90, 91])
		for (const id of FEED_ONLY_IDS) {
			expect(rarityOf(id)).toBe("legendary")
			expect(isFeedOnly(id)).toBe(true)
			expect(mult.legendary).not.toContain(id)
			expect(pairs.legendary).not.toContain(id)
			expect(feed.legendary).toContain(id)
		}
		expect([...FEED_ONLY_IDS].sort((a, b) => a - b)).toEqual([84, 85, 86, 87])
		// mnożenie/życzenia: 4 legendarne (oryginalne), zero ekskluzywnych
		expect(mult.legendary.length).toBe(4)
		// div, gap i pairs: baza + własny blok ekskluzywny (4+4)
		expect(div.legendary.length).toBe(8)
		expect(gap.legendary.length).toBe(8)
		expect(pairs.legendary.length).toBe(8)
		for (const id of PAIRS_ONLY_IDS) {
			expect(rarityOf(id)).toBe("legendary")
			expect(isPairsOnly(id)).toBe(true)
			expect(mult.legendary).not.toContain(id)
			expect(div.legendary).not.toContain(id)
			expect(gap.legendary).not.toContain(id)
			expect(pairs.legendary).toContain(id)
		}
		expect([...PAIRS_ONLY_IDS].sort((a, b) => a - b)).toEqual([80, 81, 82, 83])
		for (const id of DIVISION_ONLY_IDS) {
			expect(mult.legendary).not.toContain(id)
