...
describe("MONSTER_COUNT and MONSTERS", () => {
	test("MONSTER_COUNT === 76", () => {
		expect(MONSTER_COUNT).toBe(76)
	})

	test("MONSTERS.length === 76", () => {
		expect(MONSTERS.length).toBe(76)
	})
...
describe("rarity distribution", () => {
	test("common: 36, rare: 21, epic: 11, legendary: 8", () => {
		...
		// 4 oryginalne (45,46,47,71) + 4 tylko-dzielenie (72–75)
		expect(IDS_BY_RARITY.legendary.length).toBe(8)
	})
})

describe("DNA uniqueness", () => {
	test("all 76 DNA signatures are unique", () => {
