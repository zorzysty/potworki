...
Deterministyczny katalog 92 potworków (DNA, rzadkości, imiona) i ich rendering SVG; świat/regiony i lore.
...
- Aktualnie `MONSTER_COUNT = 92`; id 72–75 to legendary **tylko-dzielenie**, 76–79 **tylko-luka**, 80–83 **tylko-Dzielniki** (tryb `pairs`), 84–87 **tylko-porównywanie** (tryb `feed`), 88–91 **tylko-memory** (tryb `memory`); pulę losowania per tryb daje `idsByRarityForMode` (blok ekskluzywny widoczny tylko dla swojego trybu).
...
- `bun test src/monsters` + galeria `?debug` (92 sztuk, identyczne po hard-refresh).
