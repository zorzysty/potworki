...
- **`wishEggsBought` ma DRUGIEGO konsumenta poza osiągnięciami**: napędza progresję ceny Jajka Życzeń (`wishEggPrice` w `src/game/rewards.ts`, użyte w `wishEggCost`). Świadomy tradeoff — progresja nie potrzebowała nowego pola zapisu ani migracji. **Nie zerować, nie przedefiniowywać i nie usuwać tego licznika bez zmiany cennika** (wyzerowanie = jajka znów najtańsze); wycofanie osiągnięć `jajko-zyczen`/`jajka-zyczen-5` NIE zwalnia licznika.
