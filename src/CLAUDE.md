...
- Modal potworka w `CollectionScreen` jest „paszportem" **tylko dla posiadanych**: gatunek + opis z `loreFor` (`monsters/lore.ts`) i kraina pochodzenia z `originOf` (`monsters/world.ts`). Nazwę krainy pokazujemy wyłącznie gdy odblokowana (`region.stage <= unlockedStage`; inaczej mglisty teaser — chroni „tajemnicę tabliczki"); most/`BRIDGE_ORIGIN` zawsze widoczny. Nieposiadane bez zmian (sylwetka „???"). Kontener modala ma `overflow-y-auto` (długi opis).
