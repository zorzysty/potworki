...
- Metadane rzadkości (`RARITY_META`, `CARD_THEME`) w `components/rarity.ts`; style trudności osiągnięć lustrzanie w `components/achievementTier.ts`; pasek celu budowy współdzielony w `components/GoalProgressBar.tsx`; art wioski w `components/village/`; gniazdo ekranu wyklucia (tył/przód SVG wokół jajek, sloty `NEST_SLOTS` w % wrappera) w `components/NestArt.tsx`; zamiana jajka gniazdo↔dół to FLIP w `HatchScreen` (rect przed zmianą stanu → `useLayoutEffect` → sam `transform`).
