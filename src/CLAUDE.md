...
- Żadnych natywnych `<input>` w grze — iPad otwierałby klawiaturę systemową; odpowiedź renderowana w stylizowanym divie. Jedyny wyjątek: `<textarea>` importu/eksportu zapisu w `DebugScreen` (narzędzie deweloperskie, nie ekran dziecka).
...
- Zablokowana zawartość (tier Sklepiku, brama wypraw, fontanna, tryb-zabawa za bramą) to ZAJAWKA, nigdy ton błędu: nazwa i nagroda w pełnym kontraście + chip z warunkiem 🔒. Guard w store jest źródłem prawdy — UI pokazuje łagodną linijkę, nie wyszarzony przycisk.
- Przełącznik trybów na Home: rząd 1 = trzy bazowe widoki faktu, rząd 2 = tryby-zabawy odblokowywane bramami (decyzja maintainera: nowe tryby zawsze w drugim rzędzie). Wejście trybu par to `components/PairPicker` (żetony) zamiast `Keypad` — `RoundScreen` przełącza po `round.mode`, karta pytania ma własny widok w `QuestionCard`; kibic (`CheerCompanion inline`) siedzi wtedy w pionie W PRZEPŁYWIE pod kartą, bo żetony zajmują cały dół telefonu i róg zasłaniał żeton (w poziomie i w innych trybach — róg, jak dotąd). Porównywanie nie ma kolumny wejścia: odpowiedzią jest tap w potworka na karcie (`FeedCard` w `QuestionCard`; inna para potworków z kolekcji co pytanie, brakujące jako sylwetki). Etykiety, nazwy, plakietki i układ rzędów trybów: `components/modeLabels.ts`.
...
- [monsters/CLAUDE.md](monsters/CLAUDE.md) — deterministyczny katalog 88 potworków, rendering SVG, świat i lore; **ZAMROŻONY SEED — przeczytaj przed każdą zmianą w tym folderze**
