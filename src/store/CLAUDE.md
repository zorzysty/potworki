...
- Osiągnięcia: ocena czysta w `src/achievements/`; store odblokowuje (`checkAchievements` po akcjach zmieniających stan → toast; `reconcileAchievements` cicho raz przy starcie) i podbija liczniki zdarzeniowe wprost w akcjach. **Iskierki za osiągnięcie wypłaca wyłącznie `claimAchievement`** (tap dziecka na ekranie osiągnięć, flaga `claimed`) — odblokowanie nigdy nie płaci samo; to celowo: nagroda ma być odebrana, nie zauważona po fakcie.
