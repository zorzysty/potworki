...
- Pauza (`RoundState.paused`): guard w akcjach wejścia (`pressDigit`/`pressBackspace`/`pressConfirm`/`pickFactor`/`feedSide` = ciche no-op w pauzie) — jedyne miejsce wyciszające wszystkie źródła naraz (globalny `keydown` żyje obok nakładki UI). Przerwa nie może NIC kosztować.
...
- Akcje rundy (`startRound`/`startVisitRound`/`pressConfirm`/`pickFactor`/`feedSide`/`nextQuestion`) to cienkie `set({ ...patch, round })` nad `game/round.ts`; store dokłada tylko guard pauzy i `checkAchievements` po commicie. `RoundState` jest re-eksportowany stąd dla UI. W trybie par `pressDigit` przekierowuje na `pickFactor` (cyfra = żeton, 0 = 10), `pressConfirm` jest no-opem, `pressBackspace` czyści wybrany żeton, a para już znaleziona kasuje wybór zamiast go „zawieszać"; w porównywaniu cyfry 1/2 = lewy/prawy potworek (`feedSide`), `pressConfirm` no-op — klawiatura fizyczna nie ma osobnej ścieżki.
