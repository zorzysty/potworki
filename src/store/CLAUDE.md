...
- **Domknięcie rundy przechodzi przez `roundClosePatch`** (wspólny helper trzech ścieżek finalizacji: portfel przez `credit` RAZ, `totalRounds`, rozstrzygnięcie wyprawy, dni). Nowe źródło dochodu albo licznik końca rundy wchodzi TĘDY (`counterDeltas` = przyrosty, nie wartości), nigdy edycją jednej ścieżki.
...
- Jajko Życzeń losuje z puli MNOŻENIOWEJ (bez legendarnych ekskluzywnych), która domyka się przed kompletem katalogu: `wishEggAvailable` to jedyne źródło prawdy dla guardu w `buyWishEgg` i widoczności przycisku w UI; jajko kupione tuż przed domknięciem puli wykluwa duplikat (nigdy nie znika po cichu).
- `wishEggsBought` (`achievementStats`) jest ŹRÓDŁEM CENY Jajka Życzeń — nie zerować i nie przenosić bez zmiany cennika.
- Osiągnięcia: ocena czysta w `src/achievements/`; store odblokowuje (`checkAchievements` po KAŻDEJ akcji zmieniającej stan, także po `claimAchievement` — odbiór nagrody może odblokować osiągnięcie na stanie portfela → toast; `reconcileAchievements` cicho raz przy starcie) i podbija liczniki zdarzeniowe wprost w akcjach. **Iskierki za osiągnięcie wypłaca wyłącznie `claimAchievement`** (tap dziecka na ekranie osiągnięć, flaga `claimed`) — odblokowanie nigdy nie płaci samo; to celowo: nagroda ma być odebrana, nie zauważona po fakcie.
