...
- Pauza (`RoundState.paused`): guard w akcjach wejścia (`pressDigit`/`pressBackspace`/`pressConfirm`/`pickFactor`/`feedSide`/`flipCard` = ciche no-op w pauzie) — jedyne miejsce wyciszające wszystkie źródła naraz (globalny `keydown` żyje obok nakładki UI). Przerwa nie może NIC kosztować: wznowienie przesuwa zegary pytania o długość pauzy (`pauseRound`/`resumeRound` w `game/round.ts`).
...
- Jajko Życzeń losuje z puli MNOŻENIOWEJ (bez legendarnych ekskluzywnych) i jest dostępne, dopóki brakuje nielegendarnych: `wishEgg(save)` z `src/game/wishEgg.ts` to jedyne źródło prawdy dla guardu w `buyWishEgg` i przycisku w UI (store nie ma własnej logiki ceny/puli); jajko kupione tuż przed komplętem wykluwa duplikat (nigdy nie znika po cichu).
...
- Ścieżki debug rozgrywają ZWYKŁĄ rundę (`visitStage: null`), więc świadomie pomijają bonus wizyty i licznik `visitRoundsCompleted` — wizyty to feature prawdziwej gry. Panel debug zmienia zapis JEDNĄ akcją `debugPatch(patch)` (+ `checkAchievements`) nad czystymi patchami z `game/debug.ts` — nowa dźwignia panelu to nowy patch, nie nowa akcja store; `debugPatch` omija guardy gry (bramy wypraw, ceny) świadomie.
...
- Akcje rundy (`startRound`/`startVisitRound`/`pressConfirm`/`pickFactor`/`feedSide`/`flipCard`/`nextQuestion`) to cienkie `set({ ...patch, round })` nad `game/round.ts`; store dokłada tylko guard pauzy i `checkAchievements` po commicie. `RoundState` jest re-eksportowany stąd dla UI. W trybie par `pressDigit` przekierowuje na `pickFactor` (cyfra = żeton, 0 = 10; „10" wolno też wpisać jako „1"+„0" — w pytaniu z dziesiątką w parach-celach (`expectsTen`) sama „1" nigdy nie zatwierdza żetonu, tylko czeka na następną cyfrę w buforze `answer`, bez limitu czasu, jak dwucyfrowy wynik w mnożeniu), `pressConfirm` jest no-opem, `pressBackspace` czyści wybrany żeton, a para już znaleziona kasuje wybór zamiast go „zawieszać"; w porównywaniu cyfry 1/2 = lewy/prawy potworek (`feedSide`), `pressConfirm` no-op — klawiatura fizyczna nie ma osobnej ścieżki; memory jest wyłącznie dotykowe (`flipCard`, cyfry/Enter/Backspace = no-op), a `hideCards` chowa świeżo dopasowaną parę (`lastMatched`) na timer UI — pomyłkę chowa dopiero następne odkrycie.
- `setMode` odmawia trybu zamkniętego bramą (`modeUnlocked`) — guard w store jest źródłem prawdy, Home pokazuje zajawkę.
