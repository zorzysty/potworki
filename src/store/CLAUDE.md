...
- Akcje rundy (`startRound`/`startVisitRound`/`pressConfirm`/`pickFactor`/`feedSide`/`flipCard`/`nextQuestion`) to cienkie `set({ ...patch, round })` nad `game/round.ts`; store dokłada tylko guard pauzy i `checkAchievements` po commicie. `RoundState` jest re-eksportowany stąd dla UI. W trybie par `pressDigit` przekierowuje na `pickFactor` (cyfra = żeton, 0 = 10), `pressConfirm` jest no-opem, `pressBackspace` czyści wybrany żeton, a para już znaleziona kasuje wybór zamiast go „zawieszać"; w porównywaniu cyfry 1/2 = lewy/prawy potworek (`feedSide`), `pressConfirm` no-op — klawiatura fizyczna nie ma osobnej ścieżki; memory jest wyłącznie dotykowe (`flipCard`, cyfry/Enter/Backspace = no-op), a `hideCards` chowa świeżo dopasowaną parę (`lastMatched`) na timer UI — pomyłkę chowa dopiero następne odkrycie.
